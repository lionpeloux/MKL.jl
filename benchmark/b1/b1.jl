...
# GENERATE RAW DATA
function bench_b1()
    df = DataFrame(Generic = ASCIIString[], Implementation = ASCIIString[], Type = ASCIIString[], Method = Any[], CPU_mean = Float64[], CPU_conf = Float64[])
    ...
    for (t,λ,a,b,y,z) in ((Float32,λ32,a32,b32,y32,z32),(Float64,λ64,a64,b64,y64,z64))
        ...
        for (gm, imp, ty, f, args) in tup
            if length(args) == 0 # void function
                push!(df,[gm,imp, string(ty), f,0.0,0.0])
            else
    ...
    writetable(path * "b1.csv", df, separator='\t')
    df
end
...
T = Float64
df = DataFrames.readtable(path*"b1.csv", separator = '\t')
df[ df[:Type] .== string(T), :]
println(df)
...
p = Gadfly.plot(
            df, xgroup="Generic", x="Implementation", y="CPU_mean", color="Implementation",
            Geom.subplot_grid(Geom.bar, Guide.xticks(orientation=:vertical, label=false)),
            # Guide.xlabel("n-dim"),
            Guide.ylabel("CPU time in ns/element"),
            Guide.title("CPU time on n-dim vectors (n=1e"*string(convert(Int32,log10(n)))*" , $T)"),
            Theme(bar_spacing=0.1cm)
            )


draw(PNG(path*"b1_$T.png", 25cm, 12cm), p)
draw(SVG(path*"b1_$T.svg", 25cm, 12cm), p)
p
