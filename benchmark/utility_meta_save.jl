...
for v in ((a,),(b,))
    ibench(sqrt,v, 1, false ; print_stats=true, print_detail=false, print_eval=true)
end
ts(sin,(a,))
...
begin
    ...
    for i=1:7
        n = 10^i
        a = rand(n)
        res_cpu, res_gc, res_alloc = ibench(sqrt,(a,), 1, false ; print_stats=true, print_detail=false, print_eval=true)
        stat_cpu = bstat(res_cpu[2:end] ./n, 0.01)
        push!(res,stat_cpu[1])
    end
    for r = res println(round(1e9*r)) end
end
...
begin
    for i=1:7
        ...
        a = rand(n)
        sqrt(a)
        t = mean([@elapsed sqrt(a) for i=1:100])
        println(round(t*(1e9/n)))
    end
end
...
begin
    ...
    for i=1:7
        ...
        a = rand(n)
        sqrt(a)
        t = mean([@elapsed eval(:(sqrt(a))) for i=1:100])
        println(round(t*(1e9/n)))
    end
end
